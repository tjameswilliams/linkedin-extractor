...
ProfileParser.prototype.getProjects = function() {
  var $ = this.$;
  var items = this.$profile.find('#background-projects .section-item');
  return items.map(function(i, element){
    var $div = $(element);
    var projectId =  $div.attr('id').match(/\d+/)[0];
    if( $div.find('hgroup h4.summary a').length > 0 )
    {
      var projectNameNode = $div.find('hgroup h4.summary a');
      var projectName = projectNameNode.find('[dir="auto"]').text().trim();
      var projectUrl = url.parse(projectNameNode.attr('href'), true).query.url;
    }
    else
    {
      var projectName = $div.find('hgroup h4.summary').text().trim();
      var projectUrl = null;
    }
    var projectDate = $div.find('.projects-date').text().trim();
    var description = $div.find('.description').text().trim();
    if( $div.find('.associated-list').length > 0 ) {
      var membersLi = $div.find('.associated-list .associated-endorsements li');
      var members = membersLi.map(function(i, element){
        var $li = $(element);
        return {
          memberName:$li.text().trim().replace(',',''),
          url: ($li.find('a').length > 0) ? $li.find('a')['0'].attribs.href : null
        };
      }).get();
    }
    else
